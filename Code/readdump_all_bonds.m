...
%      else
      if (strncmpi(id,'ITEM: BOX BOUNDS',numel('ITEM: BOX BOUNDS')))
        ...
        if (strncmpi(id,'ITEM: NUMBER OF ENTRIES',numel('ITEM: NUMBER OF ENTRIES')))
            BondNum(i,:) = str2num(fgetl(dump));
        else
            if (strncmpi(id,'ITEM: ENTRIES index c_IntrIdx[1] c_IntrIdx[2] c_IntrIdx[3] c_IntrIdx[4] c_Dist[1] c_Dist[2] ',numel('ITEM: ENTRIES index c_IntrIdx[1] c_IntrIdx[2] c_IntrIdx[3] c_IntrIdx[4] c_Dist[1] c_Dist[2] ')))
                Bonds{i,:} = [];
                Bondsfound = [];
                for j=1:BondNum(i)
                    Bondsfound = [Bondsfound ; str2num(fgetl(dump))];
...
varargout{1}.BondNum = BondNum;
varargout{1}.Bonds = Bonds;
